...
public class CompanyOrganizationFragment2 extends FragmentPresenter<CompanyOrganizationDelegate, CommonModel> implements View.OnClickListener {
    ...
    private void initData() {
        ...
        model.findUsersByCompany((RxAppCompatActivity) getActivity(), companyId, new ProgressSubscriber<GetDepartmentStructureBean>(getActivity()) {
            @Override
    ...
    protected void bindEvenListener() {
        ...
        viewDelegate.get(R.id.search_rl).setOnClickListener(v -> {
            ...
            if (mNaviAdapter.getData().size() <= 2) {
                ...
                bundle.putString(Constants.DATA_TAG3, mNaviAdapter.getData().get(mNaviAdapter.getData().size() - 1).getOrganizationId());
            }
            bundle.putString(Constants.DATA_TAG4, mNaviAdapter.getData().get(mNaviAdapter.getData().size() - 1).getOrganizationName());
            bundle.putInt(Constants.DATA_TAG5, C.RADIO_SELECT);
            CommonUtil.startActivtiyForResult(getActivity(), SearchContactActivity.class, Constants.REQUEST_CODE1, bundle);
        });
    }
