...
public class BoardView extends HorizontalScrollView implements AutoScroller.AutoScrollListener {
    ...
    protected void onFinishInflate() {
        ...
        if (isPortrait) {
            mColumnWidth = (int) (res.getDisplayMetrics().widthPixels * 0.8875);
            interval = (int) (res.getDisplayMetrics().widthPixels * 0.15 / 4);
        } else {
        ...
        mColumnLayout.setPadding(interval, 0, interval * 2, 0);
    ...
    private int getClosestSnapColumn() {
        ...
        int count = emptyView == null ? mLists.size() : mLists.size() + 1;
        for (int i = 0; i < count; i++) {

            View listParent;
            if (emptyView != null && i == count - 1) {
                listParent = (View) emptyView.getParent();
            } else {
                listParent = (View) mLists.get(i).getParent();
            }
            int diffX = 0;
    ...
    public void scrollToColumn(int column, boolean animate) {
        if (emptyView==null && mLists.size() <= column) {
            return;
        }

        View parent;
        if (emptyView != null && column == mLists.size()) {
            parent = (View) emptyView.getParent();
        } else {
            parent = (View) mLists.get(column).getParent();
        }
        int measuredWidth = getMeasuredWidth();
        int maxScroll = mRootLayout.getMeasuredWidth() - measuredWidth;
        ...
        if (true) {
            ...
            switch (mSnapPosition) {
                case LEFT:
                    ...
                    break;
                case CENTER:
//                    if (emptyView != null && mLists.size() - 1 == column && parent.getLeft() < getScrollX()) {
//                        View emptyParent = (View) emptyView.getParent();
//                        int left = emptyParent.getLeft();
//                        int emptyWidth = emptyParent.getMeasuredWidth();
//                        newX = left - (measuredWidth - emptyWidth - interval) / 2;
//                        if (parent.getLeft() < getScrollX()) {
//                            //具有空列并且准备滑向最后一列
//                        } else {
//                            //处于空列
//                        }
//                    } else {
//                    }
                    newX = parent.getLeft() - (getMeasuredWidth() - parent.getMeasuredWidth() - interval) / 2;
                    break;
                case RIGHT:
            ...
            switch (mSnapPosition) {
                case LEFT:
                    ...
                    break;
                case CENTER:
                    newX = left - (measuredWidth - parentMeasuredWidth - interval) / 2;
                    break;
                case RIGHT:
                    newX = parent.getRight() - measuredWidth;
                    break;
                default:
                    break;
            }
            ...
            if (getScrollX() > left && left >= parentMeasuredWidth) {
                newX += parentMeasuredWidth;
            }
    ...
    public void setColumnDragEnabled(boolean enabled) {
        this.mColumnDragEnabled = enabled;
    }
     ...
     */
    public DragItemRecyclerView addColumnList(final DragItemAdapter adapter, final View header, View columnDragView, View footer, boolean hasFixedItemSize) {
        final DragItemRecyclerView recyclerView = addColumnList(adapter, header, footer, hasFixedItemSize);
        if (columnDragView != null) {
            columnDragView.setOnLongClickListener(new OnLongClickListener() {
                @Override
                public boolean onLongClick(View v) {
                    if (mColumnDragEnabled) {
    ...
    public DragItemRecyclerView addColumnList(final DragItemAdapter adapter, final View header, final View footer, boolean hasFixedItemSize) {
        ...
        layout.setPadding(interval, 0, 0, 0);
        ...
        if (emptyView == null) {
            ...
            mColumnLayout.addView(layout, mColumnLayout.getChildCount() - 1);
        }
        return recyclerView;
    }


    public void addEmptyColumn(View emptyView) {
        LinearLayout layout = new LinearLayout(getContext());
        layout.setOrientation(LinearLayout.VERTICAL);
        layout.setLayoutParams(new LayoutParams(mColumnWidth, LayoutParams.MATCH_PARENT));
        layout.setPadding(interval, 0, 0, 0);
